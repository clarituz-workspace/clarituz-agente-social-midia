...
- **[x] Integração Power BI em tempo real (pedido do usuário)** — `Workflows/PowerBiService.cs` (cliente isolado, `PostAsync` com destino fixo `api.powerbi.com`, URL vinda de `powerbi_push_url` no config — vazio = desligado); `Coletar_Metricas.xaml` envia a linha após persistir no `SM_Metricas` — falha no push só gera warn, dado local preservado; `T_ZW_ZeroWrite` refinado: escrita permitida só em arquivo `*PowerBi*` e só com destino `api.powerbi.com`; chave `powerbi_push_url` adicionada a `Data/Config.json` e ao asset `SM_Config`. Pacote **1.0.4** publicado, processo atualizado. Docs atualizados (README, manual §4.5 com passo a passo do streaming dataset, relatórios cliente/QA).
