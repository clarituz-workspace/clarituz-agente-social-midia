...
- **[x] Loop de aprendizado + imagem por formato (pedido do usuário)** — (a) `posts-monitorados.jsonl` agora grava `tema` (WorkItemData.Formato → conteudo["Tema"/"Formato"] → Pos_Aprovacao → Registrar `in_Tema`); `MetricasService.ResumirDesempenho` junta monitorados+métricas dos últimos 7 dias e produz "temas com melhor engajamento + média por plataforma"; `Calendario_Editorial` lê os arquivos do bucket (missing-safe) e injeta o bloco em `MontarPromptCalendario(nicho,tom,plataformas,dias,desempenho)` — o LLM prioriza o que performou. (b) `Curadoria` enfileira `Formato` da entrada do calendário; `Gerar_Imagem` mapeia `reels`/`stories` → 1024×1792 + hint de composição 9:16 no prompt. T_PARSE_LLM +3 assertions (resumo ordenado, vazio sem dados, bloco injetado no prompt) — verde; build Success (catch vazio bloqueado pelo ST-DBP-003 → log Trace adicionado). Pacote **1.0.8** publicado, processo atualizado.
- **[x] A/B de legenda + janela aprendida + carrossel (pedido do usuário)** — (a) `MontarPromptGeracao` pede `legendaAlternativa`; `ConteudoGerado.LegendaAlternativa` flui para o campo `legenda_alternativa` da task (humano escolhe A/B/edita). (b) `ResumirDesempenho` cruza `registradoEm` (hora UTC) × engajamento → bloco "engajamento médio por janela" (madrugada/manhã/tarde/noite) no prompt do calendário. (c) `formato=carrossel` → `imagem_carrossel_slides` (default 3, 2–10) gerações com prompt de slide numerado ("mesma identidade visual"), upload ForEach de `geradas/img-*-N.png`, `MediaUrl`=slide 1 e `conteudo["MediaUrlsCarrossel"]` com todas. T_PARSE +2 assertions (janela noite>manhã, variante parseada) — **35/35** verde. Pacote **1.0.9** publicado, processo atualizado.
- **[x] MinIO + publicadoEm real + form custom (pedido do usuário)** — (a) `MinioService.cs` (AWSSDK.S3 3.7.421.10): `posts/{postId}.json` e `metricas/{data}/{postId}.json` — PUT atômico por chave elimina race de read-modify-write e escala por prefixo; ativo quando `SM_Config.minio_endpoint` preenchido, com fallback total ao bucket legado (If MinioAtivo em Registrar/Coletar/Agendar/Calendario); credencial `MinIO_AccessKey`. (b) `ResolverMediaIdInstagramAsync` pede `timestamp` + novo `ObterCreatedTimeFacebookAsync` → `publicadoEm` real substitui `registradoEm` na janela aprendida e na `proximaColetaEm`/DeferDate (+24h da publicação, não do registro). (c) QA visual vira rubrica 0–10 (aderência/legibilidade/artefatos/segurança) com veredito derivado em código. (d) Form custom `forms/aprovacao-post.json` (EnableDynamicForms): A/B lado a lado, motivo condicional. T_PARSE: p1 com publicadoEm 21h vs registradoEm 08h prova precedência — 35/35 verde; T_ZW exceção `*Minio*` (adapter S3) — verde. Pacote **1.1.0** publicado, processo atualizado.
