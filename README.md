...
| `ImagemService.cs` | Geração de arte via OpenAI Images + **QA visual** (gpt-4o-mini com visão avalia a arte contra o briefing; reprovou → regenera 1×) — único ponto de escrita HTTP externa, destino fixo `api.openai.com`; ligado (`gerar_imagem=true`); falha não bloqueia (humano anexa mídia na aprovação) |
...
- Pacote **1.1.1** publicado; processo `clarituz-agente-social-midia` na pasta `Shared` (release `729fd7ab-7ecf-46cb-a90a-1a67c3467748`)
