...
|---|---|---|---|---|
| 2026-09-25 | 1.0 | Devin (uipath-planner) | Generated by AI Agent | SDD inicial gerado a partir da elicitação de requisitos |
...
**Need profile:** operação recorrente de social media — gerar conteúdo via LLM, **preparar** posts para Instagram/Facebook/LinkedIn (o humano aprova e publica), sugerir respostas a comentários e coletar métricas via APIs oficiais de leitura — reduzindo esforço manual e tempo de resposta; robô nunca escreve nas plataformas.
...
|---|---|---|---|---|---|
| 1 | §1 | Volume real | Confirmar posts/dia e comentários/dia reais | 3 posts/dia/plataforma, ~50 comentários/dia | no |
| 2 | §1 | Process Owner | Quem é o SME/dono do processo de social media | não registrado | no |
| 3 | §12 | Criptografia da fila | Política de dados exige criptografia de SpecificContent? | NO — sem PII | no |
| 4 | §15/§16 | Repositório Git | URL do repo fonte do projeto | não registrado | no |
| 5 | §16 | Rotação de credenciais | Owner + cadência de renovação dos tokens Meta/LinkedIn (~60d) | procedimento manual documentado | no |
| 6 | §16 | Compliance/LGPD | Autores de comentários = dados pessoais? Política de retenção de métricas | dados públicos; retenção 90 dias fila | no |
| 7 | §16 | Licenças/hosts | Confirmação de 1 unattended runtime serverless | 1 runtime | no |
| 8 | §17 | Contas de teste | Página FB/IG e LinkedIn de teste para UAT | [TEST]-tagged posts em contas reais | no |
...
| **Objective** | Automatizar a preparação e operação de social media: curadoria de pautas → geração de conteúdo por LLM → aprovação humana → **o humano publica** (o robô nunca escreve nas plataformas) → sugestão de respostas a comentários → coleta de métricas a partir do link informado |
...
- Aprovação humana obrigatória via Action Center — a task carrega o pacote completo (legenda, hashtags, mídia, plataforma, janela sugerida)
- **O humano publica**: aprovado no Action Center, o humano copia o conteúdo e publica na plataforma; cola a URL do post na task para alimentar as métricas
- Moderação de comentários: classificação de sentimento via LLM → resposta sugerida entregue ao humano via task (o robô nunca responde, oculta ou deleta)
- Coleta de métricas por post publicado (alcance, impressões, curtidas, comentários, salvamentos) a partir da URL informada → persistência em Storage Bucket
- Relatório de execução ao final de cada job

### Out of Scope

- **Publicação automatizada** — decisão de escopo: o robô nunca escreve nas plataformas; quem publica é o humano
- **Resposta/ocultação automática de comentários** — o robô sugere; o humano executa na plataforma
- DMs/mensagens diretas — exige permissão `instagram_manage_messages` com app review Meta; fase 2 [DEFAULT]
- Criação/edição de imagem e vídeo — o robô consome mídia pronta via Storage Bucket `SM_Midia`; geração de criativo é externa
- UI automation das redes sociais — tudo via API oficial (somente leitura); automação de UI violaria ToS
...
- Conta Instagram Business/Creator vinculada a uma Página do Facebook existe — **confirmado**
- App Meta com permissões de publicação/comentários **provisionado** — tokens disponíveis
- LinkedIn operado como **página da empresa** — app com escopo `r_organization_social` para leitura de comentários/estatísticas (`linkedin_org_urn` em SM_Config); a publicação é manual pelo social media admin
...
- Mídias chegam via Storage Bucket `SM_Midia` (equipe sobe o criativo; o item de fila referencia o arquivo)
...
flowchart TD
    ...
    Mode -->|AgendarMetricas| Sched[Enqueue ColetarMetricas pendentes]
    GetTx --> Route{TipoTransacao}
    Route -->|GerarConteudo| Gen[LLM gera legenda + hashtags]
    Gen --> Val[Validação compliance]
    Val --> Apr[Task Action Center — suspend/resume]
    Apr -->|Aprovado| Humano["HUMANO publica + cola URL do post"]
    Apr -->|Rejeitado| Discard[Descarta com motivo]
    Humano --> Reg[Resolve post_id da URL → enqueue ColetarMetricas]
    Route -->|ModerarComentario| Mod[LLM sentimento → task com resposta sugerida]
...
|---|---|---|
| 1 | INIT — carregar config/assets, health check Meta + LinkedIn + GenAI (somente leitura) | Orchestrator, APIs |
...
| 4 | Validação compliance — tamanho, termos proibidos, mídia | Interno |
| 5 | Aprovação — Create Form Task → Wait and Resume; humano aprova, **publica** e cola a URL | Action Center + plataforma (manual) |
| 6 | Registrar post — resolver post_id a partir da URL → enqueue ColetarMetricas | Meta Graph API, LinkedIn API (lookup), Orchestrator |
| 7 | MonitorarComentarios — polling de comentários; dedup por reference | Meta Graph API, LinkedIn API |
| 8 | ModerarComentario — sentimento LLM → task com resposta sugerida (humano posta) | UiPath GenAI, Action Center |
| 9 | AgendarMetricas — enqueue coletas vencidas/pendentes | Orchestrator |
| 10 | ColetarMetricas — GET insights → normalizar → persistir | APIs (read), Storage Bucket |
...
| 4 | Validar compliance do conteúdo | Interno | Conteúdo gerado | Aprovado interno / BusinessException | BR-01..BR-07 | B2, B3 |
| 5 | Criar task de aprovação+publicação e aguardar | Action Center | Conteúdo | Decisão + PostUrl | BR-08 | B1, B5 |
| 6 | Registrar post: resolver post_id da URL e enfileirar métricas | Meta Graph, LinkedIn, Orchestrator | PostUrl informada | item ColetarMetricas | BR-14 | E1, E3, B9 |
| 7 | Polling de comentários novos | Meta Graph, LinkedIn | since=último poll | Queue items ModerarComentario | BR-12 | E1, E3 |
| 8 | Classificar comentário e entregar resposta sugerida | GenAI, Action Center | Queue item comentário | Task com sugestão | BR-09 | E5, B8 |
| 9 | Enfileirar coletas de métricas pendentes | Orchestrator | posts registrados | Queue items ColetarMetricas | BR-12 | E3 |
...
#### Step 5 — Aprovação + publicação humana (suspend/resume)

O job cria um Form Task com o pacote completo de publicação — legenda, hashtags, plataforma, link da mídia no bucket `SM_Midia`, janela sugerida (informativo BR-06) e aviso se o limite diário sugerido já foi atingido (BR-07, informativo) — e suspende via Wait for Task and Resume. **O humano publica o post na plataforma** e registra a decisão:

- `Aprovado` — campo obrigatório "URL do post publicado" (`PostUrl`) → step 6
- `Editado` — humano edita a legenda na task; conteúdo editado é revalidado (BR-01..BR-05) antes de concluir; `PostUrl` obrigatório
- `Rejeitado` — descarta com motivo (B1)

Timeout de aprovação [DEFAULT 48h] → escalonar/abortar item (B5).

#### Step 6 — Registro do post e agendamento de métricas

O robô **não publica** — ele resolve o identificador a partir da URL informada pelo humano: Instagram (`/p/{shortcode}/` → lookup via Graph API `/ig_hashtag_search` ou oembed/media lookup [DEFAULT: parse + lookup]); Facebook (post id no permalink); LinkedIn (activity/share URN na URL). Com `post_id` resolvido → grava no Output do item e enfileira `ColetarMetricas` com `DeferDate` +24h. URL inválida ou post não encontrado → B9 (BusinessException, volta para correção humana via nova task [DEFAULT]).
...
|---|---|---|---|---|---|
| BR-01 | Limite de legenda por plataforma | Legenda não pode exceder o máximo da plataforma alvo | Geração e após edição humana | Length ≤ 2200 (Instagram), ≤ 3000 (LinkedIn), ≤ 63206 (Facebook) | 3, 4, 5 |
| BR-02 | Hashtags | Máximo de hashtags por post | Geração e validação | Count ≤ 30 (Instagram) [DEFAULT], ≤ 5 recomendado (LinkedIn) | 3, 4 |
| BR-03 | Mídia obrigatória no Instagram | Post de feed IG exige imagem ou vídeo | TipoTransacao=GerarConteudo, Plataforma=Instagram | `MediaUrl` não vazio + arquivo existe no bucket SM_Midia | 3, 4 |
| BR-04 | Formato de mídia | Mídia deve ser de tipo suportado | Validação | extensão ∈ {jpeg, png, mp4} | 4 |
...
| BR-06 | Janela sugerida de publicação | Task exibe a janela `in_JanelaPublicacao` — **informativo**: a decisão de quando publicar é do humano | Step 5 | Campo informativo na task | 5 |
| BR-07 | Limite diário sugerido | Task avisa quando `SM_ContadorDiario` ≥ `in_LimitePostsDia` — **informativo**, não bloqueia | Step 5 | Aviso na task; contador incrementa quando humano informa PostUrl | 5, 6 |
| BR-08 | Aprovação obrigatória | Nenhum conteúdo sai da fila sem decisão humana; todo post publicado é registrado pela URL | Sempre | DecisaoAprovacao ∈ {Aprovado, Editado} + PostUrl obrigatório | 5, 6 |
| BR-09 | Moderação sempre humana | O robô classifica e sugere; nunca responde, oculta ou deleta | Step 8 | Sentimento ∈ enum; Negativo/Crise → task com prioridade alta | 8 |
...
| BR-12 | Dedup de comentários/métricas | Não enfileirar comment_id ou post_id+data já visto | Steps 7, 9 | Unique reference por tipo | 7, 9 |
...
| BR-14 | PostUrl obrigatório e válido | Métricas só existem com post identificável | Step 6 | PostUrl parseável → post_id resolvido via API | 6 |
...
### Plataforma — enum interno → leitura via API (somente GET)

| Source Value | Target Value |
|---|---|
| `Instagram` | `GET /{ig-media-id}/comments`, `GET /{ig-media-id}/insights`, lookup de media por shortcode |
| `Facebook` | `GET /{post-id}/comments`, `GET /{post-id}/insights`, `GET /{page-id}/feed` |
| `LinkedIn` | `GET /socialActions/{shareUrn}/comments`, `GET /organizationalEntityShareStatistics` |

### Sentimento — classificação LLM → ação de moderação (robô só sugere)

| Source Value | Target Value |
|---|---|
| `Positivo` | `SugerirResposta` — task com resposta pronta no TomDeVoz; humano posta |
| `Neutro` | `SugerirResposta` |
| `Negativo` | `SugerirRespostaPrioritaria` — task urgente com sugestão + contexto |
| `Crise` | `SugerirRespostaPrioritaria` + alerta imediato |
| `Spam` | `SugerirOcultar` — task sugere ocultação; humano decide e executa |

### TipoTransacao — valor de fila → workflow consumer

| Source Value | Target Value |
|---|---|
| `GerarConteudo` | Gerar_Conteudo → Validar → Aprovacao (humano publica) → Registrar_Post_Publicado |
| `ModerarComentario` | Moderar_Comentario (sugestão via task) |
...
| B3 | MidiaAusenteOuInacessivel | 4 | MediaUrl vazio ou arquivo inexistente no bucket (IG) | BusinessException → Failed sem retry |
| B4 | ForaDaJanelaSugerida | 5 | Horário fora de in_JanelaPublicacao | Informativo na task (BR-06) — não é exceção; mantido como ID para telemetria |
| B5 | AprovacaoTimeout | 5 | Task sem decisão em 48h [DEFAULT] | Escalona (nova task p/ gestor) ou aborta item como Failed de negócio |
| B6 | LimiteDiarioSugeridoAtingido | 5 | Contador ≥ in_LimitePostsDia | Aviso na task (BR-07) — não bloqueia |
...
| B9 | PostUrlInvalido | 6 | URL não parseável ou post não encontrado na API | BusinessException → nova task ao humano para correção [DEFAULT] |
...
|---|---|---|---|---|---|---|
| 1 | Meta Graph API (Instagram + Facebook) | API | Direct HTTP — `https://graph.facebook.com/v21.0` | SOURCE | READ (comentários, insights, media lookup — **nunca escrita**) | Token em asset, per-run |
| 2 | LinkedIn API | API | Direct HTTP — `https://api.linkedin.com/v2` | SOURCE | READ (comentários, estatísticas — **nunca escrita**) | Token em asset, per-run |
...
| 5 | Action Center | API | Persistence activities (Form Task) | TARGET | WRITE (tasks de aprovação/sugestão) | Suspend/resume |

> **Princípio de design:** o robô só escreve em artefatos UiPath (fila, tasks, buckets, logs). Toda escrita pública nas redes é humana — elimina escopos de publicação nos apps Meta/LinkedIn e qualquer risco de post acidental.
...
| Meta Graph API | Instagram/Facebook | Direct HTTP — somente leitura (alternativa: connector `uipath-meta-facebook` existe no catálogo, não conectado) | Steps 1, 6, 7, 10 |
| LinkedIn API | LinkedIn | Direct HTTP — somente leitura (alternativa: connector `uipath-microsoft-linkedin` existe no catálogo, não conectado) | Steps 1, 6, 7, 10 |
...
│   ├── Gerar_Conteudo.xaml
│   ├── Aprovacao_Humana.xaml       (Create Form Task + Wait and Resume — suspend point; task inclui campo PostUrl e instruções de publicação manual)
│   ├── Registrar_Post_Publicado.xaml (resolve post_id da PostUrl → enqueue ColetarMetricas)
│   ├── Validar_Compliance.xaml
│   ├── Monitorar_Comentarios.xaml
│   ├── Moderar_Comentario.xaml     (classifica → task com resposta sugerida; nunca responde)
...
| 8 | `logica/Aprovacao_Humana.xaml` | Form Task (pacote de publicação + campo PostUrl) + Wait/Resume — **suspend point** | 5 | ConteudoGerado | DecisaoAprovacao, PostUrl |
| 9 | `logica/Registrar_Post_Publicado.xaml` | Resolve post_id da PostUrl via API → enqueue ColetarMetricas | 6 | PostUrl, Plataforma | post_id |
| 10 | `logica/Monitorar_Comentarios.xaml` | Poll comentários → enqueue ModerarComentario | 7 | ExecucaoContext | `out_Enqueued:int` |
| 11 | `logica/Moderar_Comentario.xaml` | Sentimento → task com resposta sugerida (sem escrita na plataforma) | 8 | WorkItemData | SugestaoModeracao |
...
└── Process (roteador)
    ...
    │         └── humano publica → Registrar_Post_Publicado → enqueue ColetarMetricas
    ├── [tx] Moderar_Comentario → ConteudoService.cs → task com sugestão
...
|---|---|---|---|---|
| `SM_WorkItems` | clarituz-agente-social-midia (self-dispatch: curadoria, monitoramento, registro pós-publicação humana) | clarituz-agente-social-midia | QUEUE_TRIGGER | 3 [DEFAULT] |
...
| **Priority / Deadline / Postpone** | ModerarComentario=High (Crise); ColetarMetricas usa `DeferDate`; sem Postpone — janela/limite são informativos ao humano (BR-06/BR-07) |
...
| `PostId` | STRING | Registrar_Post_Publicado / Agendar_Metricas | id do post na plataforma (resolvido da URL) |
| `PostUrl` | STRING | task de aprovação (humano) | URL do post publicado manualmente |
| `PostPermalink` | STRING | Registrar_Post_Publicado | permalink normalizado |

#### `SM_WorkItems` — Output / Analytics fields

| Field Name | Kind | Type | Description |
|---|---|---|---|
| `PostId` | Output | STRING | post_id resolvido da PostUrl |
| `PostUrl` | Output | STRING | URL do post publicado pelo humano |
| `AcaoModeracao` | Analytics | STRING | SugerirResposta/SugerirRespostaPrioritaria/SugerirOcultar |
...
- Aprovação: o job suspende dentro da transação via Wait and Resume (persistência). Timeout de 48h → B5. Sem risco de post duplicado porque o robô nunca publica — o humano é o escritor.
...
|---|---|---|---|
| `Meta_SystemUserToken` | Credential | Token de acesso Meta (system user, longa duração ~60 dias) | Escopos de **leitura apenas**: `instagram_basic`, `instagram_manage_comments` (leitura), `pages_read_engagement` — sem escopos de publicação (design: robô nunca escreve); renovação manual documentada |
| `LinkedIn_AccessToken` | Credential | Token OAuth LinkedIn (~60 dias) | Escopo de **leitura**: `r_organization_social` (comentários/estatísticas da página — confirmado página da empresa); `linkedin_org_urn` em SM_Config para queries de métricas |
| `SM_Config` | Text (JSON) | `ig_user_id`, `fb_page_id`, `linkedin_org_urn`, modelo LLM, calendário editorial, termos proibidos, janelas sugeridas | Por ambiente (DEV/UAT/PROD) |
...
| `SM_Midia` | Storage Bucket | Mídias aprovadas para publicação — origem confirmada; item de fila referencia o arquivo do bucket | confirmado |
...
| UAT | robot serverless | cross-platform — sem VM dedicada |
| Production | robot serverless | cross-platform — sem VM a manter (confirmado) |
...
- App Meta com escopos de **leitura** (comentários/insights); contas IG Business + FB Page vinculadas — confirmado como provisionado; review Meta é mais simples sem escopo de publicação
...
| **Machine template / VM pool** | serverless (confirmado — sem VM) |
...
|---|---|
| **Idempotency** | Unique reference por `TipoTransacao+id` (§12); métricas fazem upsert por `postId+data`; sem risco de escrita dupla nas plataformas (o robô não escreve) |
| **Checkpoint & restart** | Fila é o checkpoint — item In Progress retomado/retry pela fila; reconciliation: itens Success vs métricas gravadas |
...
| **Production smoke test** | 1 conteúdo gerado → task de aprovação → humano publica em conta de teste → URL registrada → métricas coletadas |
...
| **Availability / Resilience** | retry por item via fila; `DeferDate` para métricas; idempotência por reference |
...
2. Form Task criado com pacote completo (legenda, hashtags, mídia, janela sugerida) e campo `PostUrl`
3. Decisão `Aprovado` + PostUrl válida → `post_id` resolvido e item `ColetarMetricas` enfileirado com `DeferDate` ≥24h
4. Comentário `Positivo` gera task com resposta sugerida coerente com `TomDeVoz`; nenhuma chamada de escrita às plataformas
...
|---|---|---|---|
| B1 | Completar task como Rejeitado | humano rejeita | item Success com motivo; nenhuma métrica agendada |
| B2 | Termo proibido na legenda | validação | BusinessException, item Failed, task nunca criada |
| B3 | MediaUrl sem arquivo no bucket | validação IG | BusinessException antes da task |
| B4 | Aprovar fora da janela sugerida | — | task exibe aviso informativo; não bloqueia |
| B5 | Task sem decisão | timeout 48h | escalonamento/abort conforme regra |
| B6 | contador = limite | limite atingido | aviso informativo na task; não bloqueia |
| B9 | PostUrl inválida/curta | humano cola URL errada | task de correção ao humano; item não falha silencioso |
...
| Recovery / restart | kill job com item In Progress | item volta à fila; métricas não duplicam (upsert por postId+data) |
| Credential expiration | token Meta expirado | falha limpa no health check + alerta; nenhum token em log |
| Suspend/resume | aprovação respondida 2h depois | job suspende (robô liberado) e resume registrando o post |
| Zero-write garantido | mock das plataformas | nenhuma chamada POST/DELETE a Meta/LinkedIn durante a suíte inteira |

### UAT Acceptance Criteria

1. 5 posts completos ponta a ponta (pauta → LLM → aprovação → publicação manual pelo humano → URL colada → métricas) com 100% de aprovação humana exercida
2. 20 comentários classificados com sugestões corretas; negativo/crise sempre com prioridade alta; nenhuma escrita automatizada na plataforma
...
| Item | Decision |
|---|---|
| **Test data source** | página/conta de teste Meta + LinkedIn test page [SME REVIEW]; itens de fila sintéticos; publicação manual feita pelo humano em conta de teste |
| **Cleanup** | itens de fila de teste deletados pós-execução; posts de teste identificados com `[TEST]` e removidos pelo humano |
...
2. Primeira semana: 100% das sugestões de moderação revisadas pelo humano antes de responder, para calibrar o classificador de sentimento
...
3. Pré-requisitos externos (não-código): criar assets §15, fila §12, conexão GenAI na pasta, apps Meta/LinkedIn com escopos **somente de leitura**
